...
export const StockAdjustmentModal = ({ isOpen, onClose, lot }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl shadow-2xl border border-surface-container-low/30 w-full max-w-md">
        <div className="flex items-center justify-between p-6 border-b border-surface-container-low/20">
          <h3 className="text-xl font-black text-primary">Ajustar Stock</h3>
        ...
        <div className="p-6">
          <div className="bg-surface/30 rounded-xl p-4 mb-4">
            <p className="text-sm text-primary/60">Stock actual</p>
            <p className="text-2xl font-black text-primary">{lot.cantidad} unidades</p>
            <p className="text-xs text-primary/40 mt-1">Lote: {lot.lote}</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <p className="text-xs text-primary/40 mt-1">Valores positivos agregan, negativos retiran</p>
            </div>

            <div>
              <label className="block text-sm font-bold text-primary mb-1">Motivo *</label>
              <textarea
                value={motivo}
                onChange={(e) => setMotivo(e.target.value)}
                placeholder="Ej: Ajuste por inventario fisico, mercancia danada, etc."
                rows={3}
                className={`w-full px-4 py-2 border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 resize-none ${errors.motivo ? 'border-red-400' : 'border-surface-container-low/30'}`}
              />
