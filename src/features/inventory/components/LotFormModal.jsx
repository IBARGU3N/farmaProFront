...
export const LotFormModal = ({ isOpen, onClose, onSubmit, initialData, isLoading, productId }) => {
  ...
  useEffect(() => {
    if (initialData) {
      setForm({
        producto_id: productId || initialData.producto_id || '',
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl shadow-2xl border border-surface-container-low/30 w-full max-w-lg">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-bold text-primary mb-1">Lote *</label>
              <input
                type="text"
                value={form.lote}
                onChange={(e) => setForm({ ...form, lote: e.target.value })}
                className={`w-full px-4 py-2 border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 ${errors.lote ? 'border-red-400' : 'border-surface-container-low/30'}`}
              />
              {errors.lote && <p className="text-xs text-red-500 mt-1">{errors.lote}</p>}
            </div>
            <div>
              <label className="block text-sm font-bold text-primary mb-1">Fecha Vencimiento *</label>
              <input
                type="date"
                value={form.fecha_vencimiento}
                onChange={(e) => setForm({ ...form, fecha_vencimiento: e.target.value })}
                className={`w-full px-4 py-2 border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 ${errors.fecha_vencimiento ? 'border-red-400' : 'border-surface-container-low/30'}`}
              />
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-bold text-primary mb-1">Cantidad *</label>
              <input
                type="number"
                value={form.cantidad}
                onChange={(e) => setForm({ ...form, cantidad: e.target.value })}
                className={`w-full px-4 py-2 border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 ${errors.cantidad ? 'border-red-400' : 'border-surface-container-low/30'}`}
                min="0"
              />
              {errors.cantidad && <p className="text-xs text-red-500 mt-1">{errors.cantidad}</p>}
            </div>
            <div>
              <label className="block text-sm font-bold text-primary mb-1">Ubicacion</label>
              <input
                type="text"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-bold text-primary mb-1">Precio Compra</label>
              <input
                type="number"
            ...
            <div>
              <label className="block text-sm font-bold text-primary mb-1">Precio Venta *</label>
              <input
                type="number"
